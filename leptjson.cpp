...
static int lept_parse_string_raw(lept_context* c, char** str, size_t* len) {
	...
	*/
	...
	for (;;) {
		char ch = *p++;
		switch (ch) {
		case '\"':	// case 1：Reading ending quotation marks
			*len = c->top - head;
			*str = (char*)lept_context_pop(c, *len);
			c->json = p;
			return LEPT_PARSE_OK;
		case '\\':	// case 2：Reading escape symbol
			switch (*p++) {
			case '\"': PUTC(c, '\"'); break;
			case '\\': PUTC(c, '\\'); break;
			case '/':  PUTC(c, '/'); break;
			...
			break;
		case '\0':	// case 3：missing quotation mark（see test sample in "test_parse_missing_quotation_mark"）
			c->top = head;
			return LEPT_PARSE_MISS_QUOTATION_MARK;
		default:	// case 4：normal character
